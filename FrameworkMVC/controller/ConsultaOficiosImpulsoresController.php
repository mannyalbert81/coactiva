...
class ConsultaOficiosImpulsoresController extends ControladorBase{
	...
	public function consulta_oficios_impulsor(){
		...
		$oficios_impulsor=new OficiosModel();
		...
		$resultImpul=$oficios_impulsor->getCondiciones($columnas ,$tablas ,$where, $id);
		...
		{
			//notificaciones
			$oficios_liquidador->MostrarNotificaciones($_SESSION['id_usuarios']);
				
			$permisos_rol = new PermisosRolesModel();
			$nombre_controladores = "ConsultaOficiosImpulsores";
			$id_rol= $_SESSION['id_rol'];
			$resultPer = $oficios_impulsor->getPermisosVer("   controladores.nombre_controladores = '$nombre_controladores' AND permisos_rol.id_rol = '$id_rol' " );
			...
			if (!empty($resultPer))
			{
				...
				if(isset($_POST["buscar"])){
					...
					clientes.id_clientes = juicios.id_clientes AND usuarios.id_usuarios = oficios.id_usuario_registra_oficios AND oficios.firma_impulsor='FALSE' AND oficios.firma_secretario='FALSE'  AND oficios.firma_liquidador='FALSE'";
					...
					$resultSet=$oficios_impulsor->getCondiciones($columnas ,$tablas , $where_to, $id);
				...
				$this->view("ConsultaOficiosImpulsoresFirmar",array(
			...
			else
			{
				$this->view("Error",array(
						"resultado"=>"No tiene Permisos de Acceso a Consulta Oficios Impulsores"
	...
	public function consulta_oficios_impulsores_firmados(){
	
		session_start();
	
		//Creamos el objeto usuario
		$resultSet="";
		$oficios_impulsores=new OficiosModel();
		$usuarios = new UsuariosModel();
		// saber la ciudad del usuario
		$_id_usuarios= $_SESSION["id_usuarios"];
		...
		$id       = "asignacion_secretarios_view.id_abogado";
		$resultImpul=$oficios_impulsores->getCondiciones($columnas ,$tablas ,$where, $id);
		...
		$oficios_impulsores=new OficiosModel();
		//$oficios_impulsores=new DocumentosModel();
		...
		if (isset(  $_SESSION['usuario_usuarios']) )
		{
			...
			$nombre_controladores = "ConsultaOficiosImpulsores";
			$id_rol= $_SESSION['id_rol'];
			$resultPer = $oficios_impulsores->getPermisosVer("   controladores.nombre_controladores = '$nombre_controladores' AND permisos_rol.id_rol = '$id_rol' " );
			...
			if (!empty($resultPer))
			{
				...
				$oficios_impulsores=new OficiosModel();
					...
					$where=" juicios.id_ciudad = ciudad.id_ciudad AND juicios.id_juicios = oficios.id_juicios AND
					entidades.id_entidades = oficios.id_entidades AND
					clientes.id_clientes = juicios.id_clientes AND usuarios.id_usuarios = oficios.id_usuario_registra_oficios AND oficios.firma_impulsor='TRUE' AND oficios.firma_secretario='FALSE' AND oficios.firma_liquidador='FALSE'";
					...
					$resultSet=$oficios_impulsores->getCondiciones($columnas ,$tablas , $where_to, $id);
	
				}
	
				$this->view("ConsultaOficiosImpulsoresFirmados",array(
						"resultSet"=>$resultSet,"resultCiu"=>$resultCiu, "resultUsu"=>$resultUsu, "resultDatos"=>$resultDatos, "resultImpul"=>$resultImpul
			...
			{
				$this->view("Error",array(
						"resultado"=>"No tiene Permisos de Acceso a Consulta Oficios Liquidador Firmados"
	...
	public  function EnviarApplet()
	{
		...
		if(isset($_POST['file_firmar']))
		{
			...
			$ruta="Oficios";
			$nombreUsuario="";
			$controlador="ConsultaOficiosImpulsores";
		...
		if(isset($_POST['filesIds'])&&isset($_POST['mac'])&&isset($_POST['ruta'])&&isset($_POST['id_usuario']))
		{
			if(!is_null($_POST['filesIds']) || !is_null($_POST['mac']) || !is_null($_POST['ruta']) || !is_null($_POST['id_usuario'])){
				...
				if($permisosFirmar['estado'])
				{
					...
					$_nombre_controlador = "ConsultaOficiosLiquidador";
