<?php
class ConsultaDocumentosLiquidadorController extends ControladorBase{
	...
	public function consulta_liquidador(){
		...
		$resultSet="";
		$documentos_liquidador=new DocumentosModel();
		...
		$where    = "public.asignacion_secretarios_view.id_liquidador = '$_id_usuarios'";
			
		$id       = "asignacion_secretarios_view.id_abogado";
		$resultImpul=$documentos_liquidador->getCondiciones($columnas ,$tablas ,$where, $id);
		...
		{
			//notificaciones
			$documentos_liquidador->MostrarNotificaciones($_SESSION['id_usuarios']);
			
			$permisos_rol = new PermisosRolesModel();
			$nombre_controladores = "ConsultaDocumentosLiquidador";
			$id_rol= $_SESSION['id_rol'];
			$resultPer = $documentos_liquidador->getPermisosVer("   controladores.nombre_controladores = '$nombre_controladores' AND permisos_rol.id_rol = '$id_rol' " );
			...
			if (!empty($resultPer))
			{
				...
				if(isset($_POST["buscar"])){
					...
					$where="ciudad.id_ciudad = documentos.id_ciudad AND
						 ...
						 AND documentos.firma_impulsor ='TRUE' AND documentos.firma_secretario ='TRUE' AND documentos.firma_liquidador ='FALSE' AND asignacion_secretarios_view.id_liquidador='$_id_usuarios'";
                    ...
                    $resultSet=$documentos_liquidador->getCondiciones($columnas ,$tablas , $where_to, $id);
				...
				$this->view("ConsultaDocumentosLiquidador",array(
			...
			else
			{
				$this->view("Error",array(
						"resultado"=>"No tiene Permisos de Acceso a Firmar Documentos Liquidador"
	...
	public function consulta_liquidador_firmados(){
		...
		if (isset(  $_SESSION['usuario_usuarios']) )
		{
			...
			if (!empty($resultPer))
			{
				...
				if(isset($_POST["buscar"])){
					...
					$where="ciudad.id_ciudad = documentos.id_ciudad AND
						 ...
						 documentos.id_usuario_registra_documentos = asignacion_secretarios_view.id_abogado AND
						  clientes.id_clientes = juicios.id_clientes AND
						  estados_procesales_juicios.id_estados_procesales_juicios = documentos.id_estados_procesales_juicios
						 AND documentos.firma_impulsor ='TRUE' AND documentos.firma_secretario ='TRUE' AND documentos.firma_liquidador ='TRUE' AND asignacion_secretarios_view.id_liquidador='$_id_usuarios'";
				...
				$this->view("ConsultaDocumentosLiquidadorFirmados",array(
						"resultSet"=>$resultSet,"resultCiu"=>$resultCiu, "resultUsu"=>$resultUsu, "resultDatos"=>$resultDatos, "resultImpul"=>$resultImpul
			...
			{
				$this->view("Error",array(
						"resultado"=>"No tiene Permisos de Acceso a Consulta Documentos Liquidador Firmados"
		...
		if(isset($_GET['id']))
		{
			...
			if (! empty ( $resultDocumento )) {
				...
				try {
					...
					$this->redirect("ConsultaDocumentosLiquidador","consulta_liquidador");
	...
	public  function EnviarApplet()
	{
		...
		if(isset($_POST['file_firmar']))
		{
			...
			$ruta="Providencias";
			$nombreUsuario="";
			$controlador="ConsultaDocumentosLiquidador";
	...
	public  function FirmarProvidenciasApplet()
	{
		...
		if(isset($_POST['filesIds'])&&isset($_POST['mac'])&&isset($_POST['ruta'])&&isset($_POST['id_usuario']))
		{
			if(!is_null($_POST['filesIds']) || !is_null($_POST['mac']) || !is_null($_POST['ruta']) || !is_null($_POST['id_usuario'])){
				...
				$_nombre_tipo_notificacion="providencia_liquidador";
					...
					foreach ($array_documento as $id )
					{
						...
						if(!empty($id))
						{
							...
							try {
								...
								$firmas->UpdateBy("firma_liquidador='TRUE'", "documentos", "id_documentos='$id_providencias'");
